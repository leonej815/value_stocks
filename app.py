...
#     [Input({"type": "collapse", "index": MATCH}, "is_open"),
#      Input({"type": "tf-btn", "index": MATCH, "tf": ALL}, "n_clicks")],
#     prevent_initial_call=True
# )
# def load_chart_on_demand(is_open, btn_clicks):
#     # 1. If it's not open, return empty immediately
#     if not is_open:
#         return html.Div()

#     # 2. Get the ticker from the MATCH context
#     # Use the trigger if available, otherwise use the index from the output
#     trigger = ctx.triggered_id
    
#     # If the trigger is the collapse component, trigger['index'] is our ticker
#     # If the trigger is a button, trigger['index'] is also our ticker
#     if isinstance(trigger, dict):
#         ticker = trigger.get("index")
#         timeframe = trigger.get("tf", "2Y") # Default to 2Y if TF button wasn't the trigger
#     else:
#         # Fallback for unexpected trigger types
#         ticker = ctx.outputs_list[0]['id']['index']
#         timeframe = "2Y"

#     # 3. Create the chart
#     return create_chart(ticker, timeframe, chart_to_interval, candle_data)


# @app.callback(
#     [Output({"type": "tf-btn", "index": MATCH, "tf": ALL}, "color"),
#      Output({"type": "tf-btn", "index": MATCH, "tf": ALL}, "outline")],
#     [Input({"type": "tf-btn", "index": MATCH, "tf": ALL}, "n_clicks")],
#     prevent_initial_call=True
# )
...
#     # Find which timeframe was clicked
#     # ctx.triggered_id looks like: {"index": "AAPL", "tf": "1M", "type": "tf-btn"}
#     clicked_tf = ctx.triggered_id.get("tf", "2Y")

#     # Get the list of all timeframes for this MATCHed ticker from the output IDs
#     # This ensures we match the order of the buttons in the layout
#     button_list = ctx.outputs_list[0]
...
#         btn_tf = btn['id']['tf']
#         if btn_tf == clicked_tf:
#             colors.append("dark")    # Highlighted color
#             outlines.append(False)   # Solid fill
#         else:
#             colors.append("secondary") # Default color
#             outlines.append(True)      # Outline only
...
#     """
#     function(row_clicks, link_clicks, current_is_open) {
#         const ctx = window.dash_clientside.callback_context;
        
#         // 1. Safety check: If inputs aren't ready yet, do nothing
#         if (!ctx.triggered.length || !current_is_open) {
...
#         let triggered_id;
#         try {
#             triggered_id = JSON.parse(triggered_id_str);
#         } catch (e) {
#             return [window.dash_clientside.no_update, window.dash_clientside.no_update];
#         }
        
#         // 2. Prevent toggle if the Google Finance link was clicked
#         if (triggered_id.type === 'ticker-link') {
#             return [window.dash_clientside.no_update, window.dash_clientside.no_update];
#         }

#         const clicked_ticker = triggered_id.index;
        
#         // 3. Toggle logic: Open clicked, close others
#         const new_states = current_is_open.map((is_open, i) => {
#             let prop_id = ctx.inputs_list[0][i].id;
            
#             // If it's already an object, don't parse it
#             let row_id = (typeof prop_id === 'string') ? JSON.parse(prop_id) : prop_id;
            
#             return row_id.index === clicked_ticker ? !is_open : false;
#         });

#         const new_arrows = new_states.map(open => 
#             open ? "bi bi-chevron-up" : "bi bi-chevron-down"
#         );

#         return [new_states, new_arrows];
#     }
#     """,
#     [Output({"type": "collapse", "index": ALL}, "is_open"),
#      Output({"type": "arrow", "index": ALL}, "className")],
